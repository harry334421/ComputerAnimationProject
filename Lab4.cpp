...
//the matrix for boids, each row is one M for one boids
...
GLfloat v_leaderfollow[3] = { 0 };  
GLfloat v_flockcenter[3] = { 0 };   
GLfloat v_colliionavoid[3] = { 0 };   
GLfloat v_vmatch[3] = { 0 };   //velocity matching

// boids coloring, decided by number;
...
static GLfloat point_quaternion[100][7] = { { 1, 0, 0, 0,13,-20,4 },   //point 1
{ 0, 1, 0, 0, 0,-15,8 },  //point 2
{ 0, 0, 1, 0,-11,15,-18 },   //point 3
{ 0, 0, 0, 1,-3,0,17 }, //point 4
{ 0, 0, 1, 0,15,1,10 },    //point 5
{ 0, 1, 0, 0, -3,-1,0 },  //point 6
{ 1, 0, 0, 0, -20,0,-5 } }; //point 7 }; 
...
//Calculate the distances between two object
...
void Leader_following(int index)
{
	for (int i = 0; i < 3; i++)
	{
		v_leaderfollow[i] = (leaderM[12 + i] - boidsM[index][12 + i]) / 300;
	}
}
...
void Flock_centering(int index)
{
	...
	for (int i = 0; i < 3; i++)
	{
		v_flockcenter[i] = (center[i] - boidsM[index][12 + i]) / 200;
	}
}
	...
	for (int i = 0; i < boids_n; i++)
	{
		if (i != index)
		{
			if (Distance(boids_p[index], boids_p[i]) < 4)
			{
				for (int j = 0; j < 3; j++)
				{
					v_colliionavoid[j] = (v_flockcenter[j] - (boids_p[i][j] - boids_p[index][j])) / 200;
				}
			}
		}
	}
...
void Velocity_matching(int index)
{
	...
	for (int i = 0; i < 3; i++)
	{
		v_vmatch[i] = (average[i] - boids_v[index][i]) / 2000;
	}
}
...
void q_interpolate(GLfloat p_quaternion[100][7], GLfloat SplineM[16])
{
	...
	for (int i = 0; i < 7; i++)
	{
		...
		GLfloat GMatrix_q[4] = { p_quaternion[points][i],
			p_quaternion[(points + 1) % p_number][i],
			p_quaternion[(points + 2) % p_number][i],
			p_quaternion[(points + 3) % p_number][i] };
...
void boid_interpolate(GLfloat p_quaternion[100][3], GLfloat SplineM[16], int index)
{
	//T matrix T = {t*t*t,t*t,t,1}
	GLfloat TMatrix_q[4] = { t * t * t, t * t, t, 1 };
...
void Move_boid(int index) {
	...
	for (int i = 0; i < 3; i++)
	{
		v_leader[i] = v_leaderfollow[i];
	}
	Flock_centering(index);
	for (int i = 0; i < 3; i++)
	{
		v_flockc[i] = v_flockcenter[i];
	}
	Collision_aviod(index);
	for (int i = 0; i < 3; i++)
	{
		v_collison[i] = v_colliionavoid[i];
	}
	Velocity_matching(index);
	for (int i = 0; i < 3; i++)
	{
		v_matching[i] = v_vmatch[i];
	}
